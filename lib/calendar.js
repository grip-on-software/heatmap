...
function calendar(projectKey = "", json = []) {
    ...
    var svg = heatmapDiv.append("svg")
        .attr("width", config.calendaryearWidth * config.years.length)
    ...
    let currentYear = config.years[0];
    ...
    // Offset for months
    let baseMonthOffset = 3 * config.cellSize;
        ...
        .enter().append("text")
            ...
            .attr("transform", function(d) {
                ...
                if (d.getFullYear() != currentYear) {
                    baseMonthOffset += config.calendaryearWidth;
    ...
    rect.filter(d => d in data)
        .attr("fill", d => config.color(data[d]))
        .append("title")
        .text(d => d + ": " + data[d]);
