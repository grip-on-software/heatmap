...
class calendar {
    ...
    constructor(data = {}, configuration = {}, locales=null) {
        ...
        this.maxDomains = _.mapValues(_.zipObject(this.config.dataOptions), () => []);
    ...
    showTooltip(day) {
        ...
        if (this.config.mode === 'commits-per-developer') {
            message += `<strong>${this.locales.message("tooltip-commits-per-developer", [roundToOneDecimal(this.commitData[this.dateKey(day)] / this.developerData[this.dateKey(day)])])}</strong><br>
                ${this.locales.message("tooltip-developers", [this.developerData[this.dateKey(day)]])}<br>`;
        } else if (this.config.mode === 'total-commits') {
            message += `<strong>${this.locales.message("tooltip-commits", [this.commitData[this.dateKey(day)]])}</strong><br>`;
        } else {
            message += `<strong>${this.locales.message("tooltip-file-change", [this.filechangesData[this.dateKey(day)].length])}</strong><br>`;
        }

        if (this.config.mode !== 'file-changes') {
            message += `${this.weather[this.dateKey(day)]} °C<br>`;
        }
