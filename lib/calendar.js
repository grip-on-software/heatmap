...
class calendar {
    ...
    constructor(data = {}, configuration = {}, locales=undefined) {
        ...
        this.dayGroups.on('click', day => {
            if (this.config.mode === 'file-changes') {
                ...
                d3.select('#filechanges').append('div')
                    ...
                    .text(this.locales.message("file-change-header", [moment(filechanges[Object.keys(filechanges)[0]][0].later_date).format('LL'), filechanges.length, this.config.key]));
