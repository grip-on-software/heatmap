        ...
        this.dayGroups.on('click', day => {
            if (this.config.mode === 'file-changes') {
                ...
                d3.select('#filechanges').append('div')
                    ...
                    .text(`Na lange tijd veranderde bestanden op ${moment(filechanges[Object.keys(filechanges)[0]][0].later_date).format('LL')} (${filechanges.length})`);
                ...
                for (let repo in filechanges) {
                    if (repo !== 'length') {
                        ...
                        repoHeader.on('click', () => {
                            if (collapsed) {
                                d3.selectAll(`.repo-${repo} .file-change`).attr('style', null);
                                collapsed = false;
                            } else {
                                d3.selectAll(`.repo-${repo} .file-change`).attr('style', 'display: none');
                                collapsed = true;
                            }
                        });
