...
class calendar {
    ...
    showTooltip(day) {
        ...
        if (this.config.mode === 'commits-per-developer') {
            message += `<strong>
                ${this.locales.message("tooltip-commits-per-developer", [
                    roundToOneDecimal(this.commitData[date] /
                        this.developerData[date])
                ])}</strong><br>
                ${this.locales.message("tooltip-developers", [
                    this.developerData[date]
                ])}<br>`;
        } else if (this.config.mode === 'total-commits') {
            message += `<strong>${this.locales.message("tooltip-commits", [
                this.commitData[date]
            ])}</strong><br>`;
        } else {
            message += `<strong>${this.locales.message("tooltip-file-change", [
                _.size(this.filechangesData[date])
            ])}</strong><br>`;
        }

        if (this.config.mode !== 'file-changes' && this.weather[date] !== undefined) {
