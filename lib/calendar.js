...
class calendar {
    ...
    createDayCells() {
        ...
        this.dayGroups.on('mouseover', day => {
            tooltip.show(this.formatDate(day), 
                ...
                ${this.filechangesData[this.formatDate(day)].length} long-time files changed`)
            })
        ...
        this.dayGroups.on('click', day => {
            ...
            d3.select('#filechanges').selectAll('p').data(filechanges).enter()
                .append('p')
                .attr('class', 'file-change')
                .text(d => `${d.file}, ${d.earlier_date}`);
        });
