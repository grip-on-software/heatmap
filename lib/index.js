...
d3.json("data/commit_volume.json", function(error, commits) {
  ...
  d3.json("data/weather.json", (error, weather) => {
    ...
    const projectKeys = Object.keys(commits).sort();
      ...
      projectPicker.enter()
        ...
        .on("click", projectKey => calendar(projectKey, commits[projectKey], weather));

        // Show the calendar for the first project
        calendar(projectKeys[0], commits[projectKeys[0]], weather);
  });
});
