...
let makePage = function(commits, developers, weather) {
    let currentCalendar;
    ...
    // ALl project names
    const projectKeys = Object.keys(commits).sort(naturalSort);
    ...
    const projectsNavigation = new navigation({
        ...
        setCurrentItem: (projectKey, hasProject) => {
            ...
            currentCalendar = new calendar({
                commits: commits[projectKey], 
                developers: developers[projectKey], 
                weather: weather
            }, {
                key: projectKey,
                mode,
    ...
    currentCalendar = new calendar({
        commits: commits[projectKeys[0]], 
        developers: developers[projectKeys[0]], 
        weather: weather
    }, {
        key: projectKeys[0],
        mode,
...
axios.all([
    ...
    axios.get('data/weather.json')
        .then(weather => {
            makePage(commits.data, developers.data, weather.data);
        })
