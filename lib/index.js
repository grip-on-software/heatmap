...
let makePage = function(commits, developers, weather, projects, sources) {
    ...
    // All project data
    const projectData = (!_.isEmpty(projects) ?
    ...
    const metadata = _.keyBy(projectData, d => d.name);
    ...
    const projectsNavigation = new navigation({
        ...
        setCurrentItem: (project, hasProject) => {
            ...
            currentCalendar = buildCalendar(project);
            return true;
        },
        key: d => d.name,
        addElement: (element) => {
    ...
    currentCalendar = buildCalendar(filteredData[0] ? filteredData[0].name : '');
